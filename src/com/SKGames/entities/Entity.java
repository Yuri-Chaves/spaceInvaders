...
import com.SKGames.world.Camera;
import com.SKGames.world.Node;
import com.SKGames.world.Vector2i;
	...
	public void followPath(List<Node> path) {
		if(path != null) {
			if(path.size() > 0) {
				...
				if(x < target.x * 64) {
					x++;
				}else if(x > target.x * 64) {
					x--;
				}
				
				if(y < target.y * 64) {
					y++;
				}else if(y > target.y * 64) {
					y--;
				}
				
				if(x == target.x * 64 && y == target.y * 64) {
